...
const Footer = () => {
  ...
  const instituteName = institute?.name || "LJ University";
  const displayName = institute?.short_name || instituteName;
      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-6 mb-6">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-bold text-sm">{displayName} Placement</span>
          </div>
        ...
        <div>
          ...
          <ul className="space-y-1.5 text-gray-400 text-xs">
            {(institute?.address || institute?.placement_email || institute?.contact_phone || institute?.support_email) ? (
              <>
                {institute?.address && <li>{institute.address}</li>}
                {(institute?.placement_email || institute?.support_email) && (
                  <li>{institute.placement_email || institute.support_email}</li>
                )}
                {institute?.contact_phone && <li>{institute.contact_phone}</li>}
              </>
            ) : (
              <>
                <li>[Placement Cell]</li>
                <li>[placements@gmail.com]</li>
